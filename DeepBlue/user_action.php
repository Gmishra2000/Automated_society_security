...
if (isset($_POST['registerbtn'])) {
    ...
    if (file_exists("upload/user/" . $_FILES["person_image"]["name"])) {
        ...
        if ($password === $cpassword) {
            $query = "INSERT INTO user (username,email,houseNo,password,usertype,images) VALUES ('$username','$email','$houseNo','$password','$usertype','$images')";
    ...
    foreach ($admin_query_run as $ad_row) {
        ...
        if ($person_image == NULL) {
            ...
            if ($img_path = "upload/user/" . $ad_row['images']) {
                ...
                $_SESSION['status_code'] = "success";
                header('Location: visitor_user.php');
            }
        }
    }


    $query = "UPDATE user SET username='$edit_name', email='$edit_email',houseNo='$edit_hNo' ,password='$edit_password', images='$image_data' WHERE houseNo='$edit_hNo' ";
    $query_run = mysqli_query($connection, $query);
